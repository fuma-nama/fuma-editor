...
export function FeInput({ className, ...props }: ComponentPropsWithoutRef<"input">) {
  return (
    <input
      className={cn(
        "w-full rounded-md border border-fe-border bg-fe-input px-3 py-2 text-sm text-fe-foreground outline-none placeholder:text-fe-muted-foreground focus:border-fe-ring",
        className,
      )}
      {...props}
    />
...
export function FeAuthCard({ className, ...props }: ComponentPropsWithoutRef<"div">) {
  return (
    <div
      className={cn(
        "w-full max-w-sm rounded-lg border border-fe-border bg-fe-card p-6 text-fe-card-foreground",
        className,
      )}
      {...props}
    />
